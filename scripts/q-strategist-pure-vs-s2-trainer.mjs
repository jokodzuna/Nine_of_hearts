...
// ================================================================
// scripts/q-strategist-pure-vs-s2-trainer.mjs
// Trains q-table-strategist-pure.json against Strategist2Bot ONLY.
// Warm-starts from the existing pure table.
//
// Usage:
//   node scripts/q-strategist-pure-vs-s2-trainer.mjs [--games N] [--epsilon N] [--log-every N]
//
// Flags:
...
function playGame(eps) {
    ...
    while (!isGameOver(s) && totalMoves < STEP_LIMIT) {
        ...
        if (p !== BOT) {
            const conc = s2Bot.chooseMove(s);
            s2Bot.observeMove(s, conc);
            s = applyMove(s, conc);
            continue;
        }
...
// ---- Main loop ------------------------------------------------------
console.log(`\nQ-Strategist PURE Trainer (vs Strategist2Bot)`);
console.log(`Opponent: Strategist2Bot 100%`);
...
for (let g = 1; g <= GAMES; g++) {
    ...
    if (g % LOG_EVERY === 0) {
        ...
        console.log(
            `    vs S2(${logN}): win=${pct(logWins,logN)}` +
