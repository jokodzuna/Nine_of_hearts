// ============================================================
// strategist2-botfather.js ÔÇö Botfather variant of Strategist2Bot
//
...
export class BotfatherBot {
    constructor() {
        ...
        this._inSimulation  = false;  // true ÔåÆ skip endgame search (prevents recursion)
    }
    ...
    chooseMove(state) {
        ...
        // unconditionally ÔÇö stronger than any endgame sim or single-card play.
        ...
        // Endgame search: when either player has Ôëñ3 cards, simulate forward
        ...
        for (const oppP of opps) {
            ...
            if (this._inSimulation || state.numPlayers === 2) {
                // Full info ÔÇö read exact opp hand (2-player: state is always visible)
        ...
        // ==============================================================
        // RULE 0.5 ÔÇö Lone-Ace guard: shadow Ace out of playMoves
        // ==============================================================
        if (myAces === 1 && myTotal >= 5) {
            const nonAce = playMoves.filter(m => playRI(m) !== 5);
            if (nonAce.length > 0) playMoves = nonAce;
            else nominate(drawMove, 9000); // forced Ace ÔåÆ draw
        }

        // ==============================================================
        // RULE 1 ÔÇö 9ÔÖÑ strict opening
        // ==============================================================
        ...
        // ==============================================================
        // RULE 2 ÔÇö Triple 9s on any 9-top
        // ==============================================================
        if (topRI === 0 && my9s === 3) {
            const t9 = playMoves.find(m => playCnt(m) === 3 && playRI(m) === 0);
        ...
        // ==============================================================
        // RULE 2.5 ÔÇö Pile-trap Ace escalation
        // ==============================================================
        ...
        // ==============================================================
        // RULE 3b ÔÇö Quad K ÔåÆ Quad A finishing sequence
        // ==============================================================
        ...
        // ==============================================================
        // RULE 3c ÔÇö 4A dominance / 4K dump
        // ==============================================================
        {
            // 4A dominance: with all 4 Aces and stuck cards, force opp to draw rather than
            // dumping 4K ÔÇö opp draws A back + pile cards, uncovering lower tops for BF's stuck cards.
            ...
            // 4K dump: hand is nearly all K+A (Ôëñ1 non-KA card), junk is stuck
            // Guard: skip when BF has all 4 Aces ÔÇö playing A is dominant then.
        ...
        // ==============================================================
        // RULE 3 ÔÇö 4-of-a-kind quad dump (rank 9ÔÇôQ)
        // ==============================================================
        // R3-J-trap-Q4: opp has J+A (any combo: JA, JJAA, JKAÔÇª) near-finishing, BF has Q├ù4.
        ...
        }
        {
            ...
            for (const m of playMoves) {
                ...
                // K-quad (r=4): only in emergency ÔÇö skip here; blocksThreat/aceFinishThreat computed below
        ...
        // ==============================================================
        // RULE 3.5 ÔÇö Draw to complete a junk quad (rank 9ÔÇôQ)
        // ==============================================================
        ...
        if (drawMove !== null && myAces >= safeAceMin && stuckCount > 0
            ...
            for (let r = 0; r <= 3; r++) {
                ...
                // just play it directly to force opp to draw ÔÇö no need to inflate hand.
                if (inHand + inDraw >= 4
        ...
        // ==============================================================
        // RULE 4 pre ÔÇö Opp has 1 stuck card at non-Ace top: guard against draw freeing it.
        ...
        // ==============================================================
        // RULE 4 ÔÇö Top card is A: Ace battle
        // ==============================================================
        if (topRI === 5) {
            // Emergency: opp has 1 card ÔÇö usually play Ace to keep top high.
            // Exception: if drawing exposes a post-draw top that opp's card still can't
            // beat, draw instead ÔÇö we recover pile cards AND opp stays stuck.
            if (oppMinCards === 1) {
                let oppOnlyRI = 5;
                if (opps.length > 0) {
                    for (let r = 0; r <= 5; r++) if (state.hands[opps[0]] & RANK_MASK[r]) { oppOnlyRI = r; break; }
            ...
            // R4-K4-finish: BF has K├ù4 and Ôëñ4 non-KA junk, opp has no K or A.
            // Playing A forces opp to draw (no answer), pile resets to lower top,
            // BF deploys K├ù4 next ÔÇö unstoppable finishing sequence.
            ...
            // Ace ÔÇö mutual waste. Score 1805 beats aceScore(1800) when oppEstAces>=1 but loses
        ...
        // ==============================================================
        // RULE 5 ÔÇö Top card is K: escalation decision
        // ==============================================================
        if (topRI === 4) {
            const aceMoves  = playMoves.filter(m => playRI(m) === 5);
            ...
            // Opp has no Kings ÔÇö don't play K or they draw all pile Kings back next turn.
            // Drawing is better: BF reclaims them and opp stays K-starved.
            // Guard: skip if BF already has K├ù3 (no need for more) or opp has Ace
            // (opp will play A at K-top, not draw pile Kings ÔÇö premise invalid).
            if (drawMove !== null && drawHighCount >= 2 && oppEstKings === 0 && oppEstAces === 0
                    && myKings <= 2 && oppMinCards > 4 && stuckCount > 0)
                nominate(drawMove, 1625);

            // R5-reclaim-kings: pile has ÔëÑ2 high cards incl. a King, opp has Ôëñ2 cards with no K.
            ...
            // K-dump recovery: opp just played all Ks into pile ÔÇö draw to reclaim 3 rather than
            ...
            // Guards: skip if we have a quad, or if hand is small (Ôëñ6 cards) ÔÇö with a
            ...
            // Guard: skip if we already have a quad ÔÇö don't waste a K-play opportunity.
            ...
            // Don't play K when already K-disadvantaged ÔÇö opp will draw it back and widen the gap
            ...
            // R5-quad-K-play: BF has a winning quad ÔÇö play K to escalate even if K-disadvantaged.
        ...
        // ==============================================================
        // RULE 6 ÔÇö Top Ôëñ Q: junk-dump phase
        // ==============================================================
        if (topRI <= 3) {
            // R6-steal-pile: pile has ÔëÑ2 high/mid cards (Q+) that opp would absorb on their
            ...
            // R6-steal-jacks: pile has ÔëÑ2 drawable Jacks and opp holds Jacks.
            // If BF escalates QÔåÆA, opp's forced draw picks up those Jacks giving opp J├ù3+.
            ...
            // Guard: only draw Q back when we're already behind ÔÇö don't draw a Q
            ...
            if (topRI <= 1) {
                ...
                // Only useful when opp has Aces or Kings ÔÇö if their ceiling is Q or lower,
                // BF's K├ù4 forces draws at will, so just dump 9s/10s naturally.
                ...
                // R6-Q-skip: BF has more Qs than opp ÔÇö Q-top freezes opp's Jacks.
                ...
                // R6-K-skip: BF has strict K-advantage and opp has Jacks ÔÇö skip straight to K.
                ...
                // burn opp's Q response. Better than playing A prematurely ÔÇö saves A/K for later.
            ...
            // R6-K4-no-opp-K: BF has K├ù4 and opp has no Kings ÔÇö play K to force pile reset
            // (opp draws, pile resets) or A-exchange (BF can draw to get 4 aces next turn).
            // Applies at any top Ôëñ K where K is playable.
            ...
            // R6-K-reclaim: BF has 3K vs opp's exactly 1K ÔÇö play K to force opp to spend their
            ...
            // R6-Q-elevate-Jtop: at J-top, opp still has Jacks ÔÇö skip to Q to trap them.
            // Playing BF's J lets opp shed their J; Q-top keeps opp's Js stuck.
            // Guard: pileSize > 3 ÔÇö on shallow pile the escalated card comes right back.
            ...
            // R6-ace-escalate: opp has few cards (Ôëñ6) with Aces AND non-power junk (rank<4)
            // that will be stuck at A-top. Playing A forces an ace battle ÔÇö opp's junk stays
            ...
            // Guard: BF needs ÔëÑ2 Aces to sustain the battle + a quad to cash out after.
            ...
            // 3-ace aggression: BF has 3+ Aces vs opp's Ôëñ1 ÔÇö play A to drain opp's last ace
            ...
            // 9s that need the pile reset ÔÇö then Ace-play is still the right call.
            ...
            // freezes opp's low cards ÔÇö better than playing a King into the pile.
            ...
            if (oppMinCards <= 3 && safePlays.length > 0) {
                if (oppMinCards === 1) {
                    ...
                    // Draw to complete a quad: if drawable pile cards complete rank├ù4 for BF,
                    // draw beats playing a single ÔÇö BF gains a powerful dump.
                    ...
                    if (hRI >= 4 && myAces >= safeAceMin && (oppEstKings > 0 || oppEstAces > 0 || oppMinCards <= 2
                        ...
                        // Quad priority: play quad (rank Ôëñ K) to trap opp's low cards or force
                        // them to waste high cards ÔÇö applies before any K/A escalation logic.
                        ...
                        if (hasQuad && !(myKings > oppEstKings && oppEstKings > 0 && myAces >= safeAceMin && !_oppHasBelowQ)) {
                            ...
                            if (qm) {
                                ...
                                // At oppMinCards<=2 opp chains their last 2 cards immediately ÔÇö dump the full
                        ...
                        if (oppEstAces === 0 && oppEstKings > 0) {
                            ...
                            // topRI ÔÇö dump junk and preserve Kings rather than escalating early.
                            let _oppLowestRI = 5;
            ...
            // 6-equal-escalate: BF is strictly behind (opp has first-mover advantage) ÔÇö
            ...
            // 6-pileup: stuck 9s at 10-top ÔÇö play J/Q to raise pile above ALL opp cards.
            ...
            if (my9s >= 1 && topRI === 1) {
                ...
                if (trapMoves.length > 0) {
                    ...
                    // Don't elevate if BF has same-rank (topRI) singles to dump first ÔÇö
            ...
            // 6b: draw to unblock stuck 9s on 10-top ÔÇö only when no 10s to play and not already ahead in cards
            ...
            // 6c: K escalation ÔÇö only when stuck, pile is deep enough opp can't cheaply draw K back,
            ...
            // 6d/6e: junk singles ÔÇö lowest rank first
            // Fundamental rule: if BF has a quad at rank r, never play a single from it ÔÇö
    ...
    // ============================================================
    // Endgame search ÔÇö triggered when myCards Ôëñ 3 or oppCards Ôëñ 3
    // ============================================================
